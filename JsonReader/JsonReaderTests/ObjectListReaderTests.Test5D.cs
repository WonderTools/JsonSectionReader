...
namespace WonderTools.JsonReaderTests
{
    ...
    {
        public class Test5D : ObjectListReaderTests
        {
            [Test]
            public void Test()
            {
                var section = GetSection("objectList5");
                var result = section.GetTableAsObjectList<Employee, string, string, int, int, DateTime>(
                    (name, department, roll, age, joiningDate) => new Employee()
                    {
                        Name = name,
