...
namespace WonderTools.JsonReaderTests
{
    ...
    {
        public class Test6D : ObjectListReaderTests
        {
            [Test]
            public void Test()
            {
                var section = GetSection("objectList6");
                var result = section.GetTableAsObjectList<Employee, string, string, int, int, DateTime, int>(
                    (name, department, roll, age, joiningDate, grade) => new Employee()
                    {
                        Name = name,
